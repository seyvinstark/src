...
export function ClassTimetableDocument({
  state,
  gradeId,
  ...
  const grade = state.grades.find((g) => g.id === gradeId)?.label ?? "Grade";
  ...
  const entryAt = (day: Day, slotId: Id) =>
    state.entries.find((e) => e.gradeId === gradeId && e.day === day && e.slotId === slotId);
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <Text style={styles.who}>{grade.toUpperCase()}</Text>
          ...
          {keys.map((startEnd) => (
            <View key={startEnd} style={styles.row}>
              ...
              {days.map((d, i) => {
                ...
                const teacher = state.teachers.find((t) => t.id === entry.teacherId)?.name ?? "?";
                return (
                  <View key={d} style={style}>
                    <Text style={styles.cellTitle}>{subj}</Text>
                    <Text style={styles.cellSub}>{teacher}</Text>
                  </View>
                );
              })}
            </View>
